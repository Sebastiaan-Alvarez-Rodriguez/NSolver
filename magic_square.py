import numpy as np
from ga_skeleton import ga_skeleton
...
def eval_cube(cube):
    ...
    errors = np.concatenate([
        _calc_cube_errors(cube, magic_constant),
        _calc_space_square_diag_errors(cube, magic_constant)
    ])
...
def _verify_square(square):
    ...
    if np.round(n) ** 2 != len(square):
        raise ValueError(f'Invalid length! The solution length should be a square number\n'
                         f'  now: {len(square)}\n')
    else:
        n = int(np.round(n))

    numbers = set(range(1, n**2+1))
    if len(set(square) - numbers) != 0:
        raise ValueError(f'Invalid representation! The solution should be a permutation of 1,...,{n**2}\n'
                         f'  Missing numbers: {numbers - set(square)}\n'
                         f'  Numbers that do not belong: {set(square) - numbers}')
...
def _verify_cube(cube):
    ...
    numbers = set(range(1, n**3+1))
    if len(set(cube) - numbers) != 0:
        raise ValueError(f'Invalid representation! The solution should be a permutation of 1,...,{n**3}\n'
                         f'  Missing numbers: {numbers - set(cube)}\n'
                         f'  Numbers that do not belong: {set(cube) - numbers}')
...
if __name__ == '__main__':
    ...
    try:
        ...
        all_passed = False
    print('Test passed') if all_passed else print('Test failed')